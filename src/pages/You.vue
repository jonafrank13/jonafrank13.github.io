  ...
  <q-page class="flex content-center flex-center contact-page q-py-md q-px-xl">
    <div class="content text-center" :class="$q.dark.isActive ? 'bg-primary' : ''">
      ...
      <template v-if="userData.region">
        <p>
          <span>Hi , It seems like you are accessing the my website from a the ip address 📡 <i>{{userData.query}}</i>, your internet service provider is <i>{{userData.isp}}</i></span><br>
          ...
          <span>I sense that you are currently at &nbsp;🌎<i>{{userData.lat}}&#176;</i> latitude &amp; <i>{{userData.lon}}&#176;</i> longitude, which is around 🗺️<i>{{userData.city}},</i>&nbsp;<i>{{userData.regionName}},</i>&nbsp;<i>{{userData.country}}</i><span v-if="userData.zip"> with the zipcode <i>{{userData.zip}}</i></span></span><br>
        </p>
