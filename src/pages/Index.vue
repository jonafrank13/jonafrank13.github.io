  ...
  <q-page class="flex row flex-center justify-center content-center front-page q-pa-sm">
    ...
    <q-page-sticky position="bottom-right" :offset="[15, 15]">
      <q-fab ripple color="secondary" padding="sm" icon="add_link" direction="left" @click="vibrate();">
        ...
        <q-fab-action v-if="canShare" padding="sm" color="warning" @click="vibrate();sharePage()" icon="ios_share">
      ...
      <q-card style="min-width: 350px">
        ...
        <q-card-section>
          ...
          <a title="Click to open the link via an app" :href="upiLink" target="_blank">
            <q-img
              class="q-mx-auto row items-center content-center justify-center"
              style="max-width: 200px"
              :src="`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(upiLink)}&color=5dc855&bgcolor=1a2936`"
            />
          </a>
...
export default defineComponent({
  ...
  setup () {
    ...
    const payModel = ref(false)
    const amount = ref('50.00')
    const note = ref('Enjoy your Coffee!')

    const upiLink = computed(() => {
      return `upi://pay?pa=jona.mailbox@okhdfcbank&pn=jona%20frank&am=${amount.value}&tn=${note.value}&cu=INR`
    })
...
.img-box::before {
  ...
  width: 100%;
  height: 100%;
  background: url('/JONA.jpg') no-repeat;
